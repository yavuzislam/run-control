...
public class Ozellestirme_Manager : MonoBehaviour
{
    ...
    public List<ItemBilgileri> _ItemBilgileri = new List<ItemBilgileri>();//_ItemBilgileri adýnda liste oluþturduk
    ...
    void DurumKontrolEt(int Bolum, bool islem = false)
    {
        ...
        {
            #region
            if (_BellekYonetim.VeriOku_i("AktifSapka") == -1)//yani önceden sapka sewçilmediyse yada oynandýktan sonra kaldýrýldýysa
            {
        ...
        {
            #region
            if (_BellekYonetim.VeriOku_i("AktifSopa") == -1)//yani önceden sapka sewçilmediyse yada oynandýktan sonra kaldýrýldýysa
            {
        ...
        {
            if (_BellekYonetim.VeriOku_i("AktifTema") == -1)//yani önceden sapka sewçilmediyse yada oynandýktan sonra kaldýrýldýysa
            {
                if (!islem)
                {
                    TextObjeleri[5].text = SatinAlmaText;
        ...
        if (AktifislemPaneliIndex != -1)
        {
            switch (AktifislemPaneliIndex)
            {
                case 0:
                    ...
                    //SatinAlmaText.text = SatinAlmaText;   önceden burasý vardý sonradan satinalmasonuc fonksiyonu ile revize edildi
                    //islemButonlari[0].interactable = false;
                    //islemButonlari[1].interactable = true;
                    //PuanText.text = _BellekYonetim.VeriOku_i("Puan").ToString();
                    break;
